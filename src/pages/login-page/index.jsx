...
function LoginPage({ onToggleTheme }) {
  ...
  const handleSubmit = async (event) => {
    ...
    if (!allValid) {
      if (!currentValidation.length)
        setError("Password must be 8 characters at least.");
      else if (!currentValidation.capital)
        setError("Password must contain at least one uppercase letter.");
      else if (!currentValidation.number)
        setError("Password must contain at least one number.");
      else if (!currentValidation.symbol)
    ...
    <div className="login-container">
      ...
      <form className="login-form" onSubmit={handleSubmit} noValidate>
        ...
        <ul className="password-criteria">
          ...
          <li className={validation.number ? "valid" : "invalid"}>number</li>
